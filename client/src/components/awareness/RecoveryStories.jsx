...
function StoryBlock({ title, items, variant = "bullet" }) {
  ...
  return (
    <div className="mt-4">
      ...
      {variant === "text" ? (
        <p className="mt-2 text-sm leading-relaxed text-slate-600">{items}</p>
      ) : (
    ...
    >
      <Card className="overflow-hidden p-0">
        <div className={`bg-gradient-to-br ${story.theme} px-6 py-5`}>
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-600/90">Recovery story</p>
          <h3 id={`${story.id}-title`} className="mt-2 font-display text-2xl text-slate-900">
            {story.title}
          </h3>
        ...
        <div className="space-y-1 px-6 py-6">
          ...
          <div className="mt-5 rounded-2xl border border-teal-100/80 bg-teal-50/50 p-4">
            <div className="flex items-start gap-2">
              <HiOutlineSparkles className="mt-0.5 h-5 w-5 shrink-0 text-teal-700" aria-hidden />
              <p className="text-sm leading-relaxed text-slate-700">
                <span className="font-semibold text-slate-900">Reflection: </span>
                {story.conclusion}
              </p>
            </div>
          </div>
    ...
    <section className="mt-20" aria-labelledby="recovery-heading">
      ...
      <div className="mb-8 flex flex-wrap justify-center gap-3">
        {recoveryAffirmations.map((line) => (
          <span
            key={line}
            className="inline-flex items-center gap-2 rounded-full border border-teal-200/80 bg-white/70 px-4 py-2 text-sm font-medium text-teal-900 shadow-sm"
          >
      ...
      <div className="mt-10 flex flex-col items-center gap-4 rounded-[1.75rem] border border-white/70 bg-gradient-to-r from-teal-50/80 via-white/60 to-violet-50/80 p-8 text-center shadow-lg">
