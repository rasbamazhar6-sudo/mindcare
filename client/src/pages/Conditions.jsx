...
const BODY_TEXT = "text-[15px] leading-relaxed text-slate-600 dark:text-slate-400";

const BulletList = memo(function BulletList({ items, dotClass }) {
  return (
    <ul className="mt-1.5 space-y-1">
      {items.map((s) => (
        <li key={s} className={`flex gap-2 ${BODY_TEXT}`}>
...
const ConditionCard = memo(function ConditionCard({ condition, isOpen, onToggle }) {
  const panelId = `panel-${condition.id}`;
  const headingId = `heading-${condition.id}`;

  return (
    <article className="overflow-hidden rounded-2xl border border-white/70 bg-white/55 shadow-sm backdrop-blur transition-all duration-300 hover:shadow-md dark:border-slate-700/70 dark:bg-slate-800/55 dark:hover:shadow-lg dark:hover:shadow-black/20">
      <button
        type="button"
        id={headingId}
        ...
        <div className="min-h-0 overflow-hidden">
          {isOpen ? (
            <div className="space-y-4 border-t border-slate-200/60 px-4 pb-4 pt-3 dark:border-slate-700/60 sm:px-5">
              <section>
                <h3 className="text-[10px] font-semibold uppercase tracking-wide text-slate-800 dark:text-slate-200">
                  Overview
                </h3>
                <p className={`mt-1 ${BODY_TEXT}`}>{condition.overview}</p>
              </section>
              <div className="grid gap-4 sm:grid-cols-2">
                <section>
                  ...
                  <BulletList items={condition.symptoms} dotClass="bg-violet-400" />
                </section>
                <section>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wide text-slate-800 dark:text-slate-200">
                    Possible causes
                  </h3>
                  <BulletList items={condition.causes} dotClass="bg-teal-400" />
                </section>
                <section>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wide text-slate-800 dark:text-slate-200">
                    Coping methods
                  </h3>
                  <BulletList items={condition.coping} dotClass="bg-sky-400" />
                </section>
                <section>
                  <h3 className="text-[10px] font-semibold uppercase tracking-wide text-slate-800 dark:text-slate-200">
                    Precautions
                  </h3>
                  <BulletList items={condition.precautions} dotClass="bg-amber-400" />
                </section>
              </div>
